    ...
    try {
      const res = await fetch('/api/sentiment/generate-council-memo', {
        ...
        body: JSON.stringify({
          ...
          jurisdiction,
          stats,
          committeeName: selectedBill.hearing?.committee || 'City Council Committee of the Whole',
        }),
      });
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`text-[10px] font-mono mt-1.5 flex items-center justify-between ${isSelected ? 'text-[#aaa]' : 'text-[#777]'}`}>
                      <span>{b.hearing?.committee ? b.hearing.committee.replace('Committee on ', '') : 'Council Review'}</span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-2 text-xs font-mono pt-2 border-t border-[#1A1A1A]/20">
                  <div>
                    ...
                    <span className="font-bold text-[#1A1A1A]">
                      {memo?.memoHeader?.to || selectedBill.hearing?.committee || 'Members of City Council'}
                    </span>
                  </div>
                  <div>
