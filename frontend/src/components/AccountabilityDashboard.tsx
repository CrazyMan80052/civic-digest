    ...
    <div className="space-y-6">
      ...
      <div className="bg-[#FDFDFC] border border-[#1A1A1A]/20 p-6 shadow-xs">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4 border-b border-[#1A1A1A]/10 pb-4">
          ...
          <div className="flex items-center gap-2" role="group" aria-label="Policy outcome case studies">
            {POLICY_CORRELATIONS.map((c, idx) => (
              <button
                type="button"
                key={c.policyBillId}
                onClick={() => setSelectedCorrelationIdx(idx)}
                aria-pressed={selectedCorrelationIdx === idx}
                aria-label={`View case study #${idx + 1}: ${c.policyTitle}`}
                className={`text-xs font-bold uppercase tracking-wider px-3 py-1.5 border transition-colors focus:outline-hidden focus-visible:ring-2 focus-visible:ring-[#1A1A1A] ${
        ...
        <div className="mt-4 p-4 bg-[#F2F0EA] border-l-4 border-[#2D6A4F] flex items-start gap-3">
          <CheckCircle2 className="w-5 h-5 text-[#2D6A4F] shrink-0 mt-0.5" aria-hidden="true" />
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between border-b border-[#1A1A1A]/20 pb-2">
          <h3 className="font-serif font-medium text-xl text-[#1A1A1A] flex items-center gap-2">
            <Users className="w-4 h-4 text-[#1A1A1A]" aria-hidden="true" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {OFFICIALS.map((person) => (
            ...
            >
              <div>
                <div className="flex items-start justify-between gap-2 mb-3">
                  <div>
                    ...
                    <span className="text-xs text-[#4B5563] font-mono font-medium block">
                      {person.title}
                    </span>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-2 bg-[#F2F0EA] p-2.5 border border-[#1A1A1A]/10 text-center mb-3 text-xs">
                  <div>
                    <span className="text-[9px] text-[#525252] uppercase font-bold tracking-wider font-mono block">Attendance</span>
                    <span className="font-bold font-mono text-[#2D6A4F] text-sm">{person.votingAttendanceRate}%</span>
                  </div>
                  <div>
                    <span className="text-[9px] text-[#525252] uppercase font-bold tracking-wider font-mono block">Sponsored</span>
                    <span className="font-bold font-mono text-[#1A1A1A] text-sm">{person.sponsoredBillsCount}</span>
                  </div>
                  <div>
                    <span className="text-[9px] text-[#525252] uppercase font-bold tracking-wider font-mono block" title="Frequency of voting YES on bulk consent calendar items">Consent Yes</span>
                ...
                <div className="space-y-1.5 mb-3">
                  <span className="text-[9px] uppercase font-bold text-[#525252] block tracking-widest font-mono">
                    Key Policy Focus Areas
                  </span>
                  <div className="flex flex-wrap gap-1">
                    {person.keyPriorities.map((p) => (
                      <span key={p} className="text-[10px] font-mono bg-[#FDFDFC] border border-[#1A1A1A]/20 text-[#374151] px-2 py-0.5">
                        {p}
                      </span>
                    ))}
                  </div>
              ...
              {/* Contact Footer */}
              <div className="pt-3 border-t border-[#1A1A1A]/10 flex items-center justify-between text-xs text-[#4B5563]">
                <a
                  href={`mailto:${person.email}`}
                  aria-label={`Email office of council member ${person.name}`}
                  className="hover:text-[#1A1A1A] font-bold uppercase tracking-wider text-[10px] flex items-center gap-1 font-mono focus:outline-hidden focus-visible:ring-2 focus-visible:ring-[#1A1A1A]"
                >
